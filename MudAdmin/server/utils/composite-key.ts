...
export function requireCompositeKey(event: H3Event, table: string): Record<string, unknown> {
  const raw = getRouterParam(event, 'id')
  if (!raw) {
    throw createError({ statusCode: 400, statusMessage: `Missing composite key for ${table}` })
  }
  return decodeCompositeKey(table, raw)
}
